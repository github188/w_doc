...
```c++
//新增缓存
vector<string> m_vecExceptUserDomain  //异常用户域列表
```
...
op3=>operation: 创会成功往会议源平台域的confinfodb添加数据
...
> 平台下发给MS,PAS的LICENSE需要累加其他"服务平台列表"平台的LICENSE数
...
>
> 不考虑主平台异常，主平台的主平台也异常，主平台的主平台LICENSE更新的情况
...
> MCU_MAU_CONFSTATECHANGE_NTF直接在cmussn:DaemonInstance中处理(包括会议名和密码修改通知),因其解析出的MCU:IP不在本平台
>
...
> 平台间切换灾备之后，在备平台会管也要同样更新预约会议，个人模板，固定虚拟会议室E164号到源平台的REDIS中，CSS也要订阅服务平台的REDIS的发布通道
...
>
> 与MCU短暂断链了，此时可能服务平台的REDIS上的该会议是待恢复,则仍将该会议归属于灾备平台MCU，若会议是正创建或正召开状态，会将本平台的MCU会议结掉( 当前逻辑已能处理)
